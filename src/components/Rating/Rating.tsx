...
export function Rating({ value, max = 5 }: { value: number; max?: number }) {
	...
	for (let i = 0,remaining=value; i < stars.length; i++,remaining=remaining-1) {
		const imageSrc = remaining > 1 ? "/star.svg" : remaining >= 0.5? "/star-half-dark.svg" : "/star-empty-dark.svg";
		stars[i] = (
