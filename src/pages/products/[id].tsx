...
import { CardBig, SkeletonCardFocused } from '@/components/Card';
...
export const getStaticProps: GetStaticProps<{ dehydratedState: DehydratedState, id:string }, { id: string }> = async function (context) {
  const queryClient = new QueryClient()
	const {params} = context
	if (!params) {
		throw new Error('No param id');
	}else{
		await queryClient.prefetchQuery<Product, { code: number; message: string }>(['product',params.id],()=>getProduct(params.id))
		return {
			props: {
	...
	return (
		<>
			<Head>
				...
				{product && <meta name='description' content={`Product - ${product.description}`} />}
			</Head>
			<Layout>
				{product && <CardBig product={product} />}
				{isLoading && <SkeletonCardFocused/>}
